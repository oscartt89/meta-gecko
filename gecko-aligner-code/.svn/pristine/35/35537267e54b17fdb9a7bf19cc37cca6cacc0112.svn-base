...
int HitsControl(int ac, char **av) {
	...
	fprintf(fInf, "Min.fragment.length  : %d\n", Lmin);
	fprintf(fInf, "Min.Identity         : %d\n", SimTh);
...
int FragFromHit(long M[1000][100], struct FragFile *myF, hit *H, struct Sequence *sX,
		uint64_t n0, uint64_t nsx, struct Sequence *sY,
		uint64_t n1, uint64_t nsy, int Lm, int SimTh, int WL,
		int fixedL, char strand) {
	...
	int minLength =
			(fixedL) ?
					Lm :
					(int) (min(getSeqLength(sX, H->posX, nsx),
