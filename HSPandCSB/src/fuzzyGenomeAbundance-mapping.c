/*
 * fuzzyGenomeAbundance-mapping.c
 *
...
int seleccion(int* ident_genomes,int ngenomes,int* genomes,int* genomes2,int* genomes3);
...
int main(int ac,char** av){
	...
	*/
	...
	// creamos matriz de fragmentos. nf * genomes
	int* indice_mapping = (int*)malloc(sizeof(int)*ngenomes);
	int** mapping = (int**)malloc(sizeof(int*)*ngenomes);
	for(i=0;i<ngenomes;i++){
		indice_mapping[i]=0;
		mapping[i] = (int*)malloc(sizeof(int)*nf);
		if(mapping[i]==NULL){printf("Error, ver linea 187\n");exit(-1);}
		for(j=0;j<nf;j++)mapping[i][j]=-1;
	}
			...
			if( (readX_ant!=readX) || (genomeY_ant != genomeY) ){
				...
				CONT++;
					...
					switch (OPT){
						...
						case 'i': indice=seleccion(ident_genomes,ngenomes,genomes,genomes2,genomes3); break;
						case 'c': indice=seleccion(cov_genomes,ngenomes,genomes,genomes2,genomes3); break;
						case 's': indice=seleccion(score_genomes,ngenomes,genomes,genomes2,genomes3); break;
						case 'g': indice=seleccion(sim_score_genomes,ngenomes,genomes,genomes2,genomes3); break;
						default: indice=seleccion(ident_genomes,ngenomes,genomes,genomes2,genomes3);
					}
					// Indice nos devuelve el genoma ganador para un read determinado.
					mapping[indice][indice_mapping[indice]++]=readX_ant;
