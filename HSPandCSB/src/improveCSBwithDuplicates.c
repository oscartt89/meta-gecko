...
int NewListDuplicates(Lista* lista,Fragmentv3* f,int nf);
...
// Constante de tolerancia de overlapping para duplicados
int SOL=1; // Number between 0-100
...
int main(int ac,char** av){
	...
	// Duplicates
	int i,j;
	int nd=0; // Number of duplication
	for(i=0;i<nf;i++){
		if(f[i].block>=0){
			for(j=0;j<nf;j++){
				if(f[j].block>=0){
					if(overlapFragmentv3(f[i],f[j],'x',SOL) || overlapFragmentv3(f[i],f[j],'y',SOL)){
						if(f[i].duplications==0)f[i].duplications=nd++;
						f[j].duplications=f[i].duplications;
	...
	Lista lista = NULL;
	size_of_list=NewListDuplicates(&lista,f,nf);
//+++++++++++++++++++++++++++	
/************ START PROGRAM ***************************************************************/
	int n=1;
...
int NewListDuplicates(Lista* lista,Fragmentv3* f,int nf){
		...
		for(i=0;i<nf;i++){
			...
			if(f[i].block>=0 && f[i].duplications>0){
				size_of_list++;
