...
int main(int ac,char** av){
	...
	*/
	...
	int score=0;
       ...
       if(HEADER) printf("R\tG\tCover\tLenR\ttotSco\ttotID\ttotLEN\t%%ID\tnF\tfrags\n");
		...
		// Si no esta solapado
			...
			if( (readX_ant!=readX) || (genomeY_ant != genomeY) ){
				...
				if( (Totcov) ){ // *100> U
					...
					printf("%d\t%d\t%.2f\t%d\t%d\t",readX_ant,genomeY_ant,Totcov,maxX,score);
				...
				// new round
				if(readX_ant != readX)maxX=f[i].xEnd;
